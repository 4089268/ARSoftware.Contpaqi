...
{
    public class MovimientoServiceFE(IProductoService pService) : IMovimientoService
    {
        ...
        public void ActualizarMovimiento(Movimiento movimiento)
        {
            ...
            FacturaElectronicaSdk.fBuscarIdMovimiento(movimiento.Id).TirarSiEsError();

            // Activar el modo de edición
            FacturaElectronicaSdk.fEditarMovimiento().TirarSiEsError();

            // Actualizar los campos del registro donde el SDK esta posicionado
            FacturaElectronicaSdk.fSetDatoMovimiento("COBSERVAMOV", movimiento.Observaciones).TirarSiEsError();

            // Guardar los cambios realizados al registro
            FacturaElectronicaSdk.fGuardaMovimiento().TirarSiEsError();
        }
        ...
        public Movimiento BuscarMovimientoPorId(int movimientoId)
        {
            ...
            FacturaElectronicaSdk.fBuscarIdMovimiento(movimientoId).TirarSiEsError();
        ...
        public List<Movimiento> BuscarMovimientosPorFiltro(int documentoId)
        {
            ...
            // Cancelar filtro
            FacturaElectronicaSdk.fCancelaFiltroMovimiento().TirarSiEsError();

            // Filtrar movimientos
            FacturaElectronicaSdk.fSetFiltroMovimiento(documentoId).TirarSiEsError();

            // Posicionar el SDK en el primer registro
            FacturaElectronicaSdk.fPosPrimerMovimiento().TirarSiEsError();

            // Leer los datos del registro donde el SDK esta posicionado
            movimientosList.Add(LeerDatosMovimiento());

            // Crear un loop y posicionar el SDK en el siguiente registro
            while (FacturaElectronicaSdk.fPosSiguienteMovimiento() == SdkConstantes.CodigoExito)
            {
                // Leer los datos del registro donde el SDK esta posicionado
                movimientosList.Add(LeerDatosMovimiento());
                ...
                if (FacturaElectronicaSdk.fPosMovimientoEOF() == 1) break;
            }
        ...
        public int CrearMovimiento(Movimiento movimiento)
        {
            ...
            FacturaElectronicaSdk.fAltaMovimiento(movimiento.DocumentoId, ref nuevoMovimientoId, ref nuevoMovimiento).TirarSiEsError();
        ...
        public void EliminarMovimiento(Movimiento movimiento)
        {
            ...
            // Eliminar movimiento
            FacturaElectronicaSdk.fBorraMovimiento(movimiento.DocumentoId, movimiento.Id).TirarSiEsError();
        }
        ...
        private Movimiento LeerDatosMovimiento()
        {
            ...
            // Leer los datos del registro donde el SDK esta posicionado
            FacturaElectronicaSdk.fLeeDatoMovimiento("CIDMOVIMIENTO", idBd, 3000).TirarSiEsError();
            FacturaElectronicaSdk.fLeeDatoMovimiento("CIDDOCUMENTO", documentoIdBd, 3000).TirarSiEsError();
            FacturaElectronicaSdk.fLeeDatoMovimiento("CIDPRODUCTO", productoIdBd, 3000).TirarSiEsError();
            FacturaElectronicaSdk.fLeeDatoMovimiento("CUNIDADES", unidadesBd, 3000).TirarSiEsError();
            FacturaElectronicaSdk.fLeeDatoMovimiento("CPRECIO", precioBd, 3000).TirarSiEsError();
            FacturaElectronicaSdk.fLeeDatoMovimiento("CREFERENCIA", referenciaBd, 3000).TirarSiEsError();
            FacturaElectronicaSdk.fLeeDatoMovimiento("COBSERVAMOV", observacionesBd, 3000).TirarSiEsError();
            FacturaElectronicaSdk.fLeeDatoMovimiento("CTOTAL", totalBd, 3000).TirarSiEsError();
